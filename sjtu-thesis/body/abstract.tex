...
首先，对于高速移动网络GSM-R而言，其首要因素是保证其系统可靠性，因此在满足测量精度的前提先应尽量降低其测试开销，实现对GSM-R网络的在线实时测试，保证整个高铁系统的稳定运行。而GSM-R网络具有移动终端高速移动性和无线传播环境复杂性的特点，从而对其通信质量测试形成巨大挑战。传统的信号强度测试算法无法直接应用于GSM-R网络测试中，这就要求GSM-R网络信号强度测试算法必须具有实时高效的特点，以适应高速铁路的特殊要求，即在高速移动条件下满足信道状态测试精度并降低测试开销。
